...
`)}var ft=null;function Yt(o){return(!ft||o)&&(ft=new We(o)),ft}function Es(o){let t=Yt(o);return t.start(),t}function Ds(){ft?.stop()}async function Pr(o,t){let e=o instanceof ArrayBuffer?o:await Jt(o),n=e.byteLength,s,i=0,r=0;switch(t.method){case"int8":({data:s,layersQuantized:i,layersSkipped:r}=_r(e,t));break;case"uint8":({data:s,layersQuantized:i,layersSkipped:r}=Lr(e,t));break;case"float16":({data:s,layersQuantized:i,layersSkipped:r}=Nr(e,t));break;case"int4":({data:s,layersQuantized:i,layersSkipped:r}=Cr(e,t));break;default:s=e}return{modelData:s,originalSize:n,quantizedSize:s.byteLength,compressionRatio:n/s.byteLength,stats:{layersQuantized:i,layersSkipped:r}}}async function Jt(o){return new ArrayBuffer(0)}function _r(o,t){let e=new Float32Array(o),n=new Int8Array(e.length),s=0;for(let r=0;r<e.length;r++){let a=Math.abs(e[r]??0);a>s&&(s=a)}let i=s/127;for(let r=0;r<e.length;r++)n[r]=Math.round((e[r]??0)/i);return{data:n.buffer,layersQuantized:1,layersSkipped:0}}function Lr(o,t){let e=new Float32Array(o),n=new Uint8Array(e.length),s=1/0,i=-1/0;for(let a=0;a<e.length;a++){let c=e[a]??0;c<s&&(s=c),c>i&&(i=c)}let r=(i-s)/255;for(let a=0;a<e.length;a++)n[a]=Math.round(((e[a]??0)-s)/r);return{data:n.buffer,layersQuantized:1,layersSkipped:0}}function Nr(o,t){let e=new Float32Array(o),n=new Uint16Array(e.length);for(let s=0;s<e.length;s++)n[s]=Or(e[s]??0);return{data:n.buffer,layersQuantized:1,layersSkipped:0}}function Cr(o,t){let e=new Float32Array(o),n=new Uint8Array(Math.ceil(e.length/2)),s=0;for(let r=0;r<e.length;r++){let a=Math.abs(e[r]??0);a>s&&(s=a)}let i=s/7;for(let r=0;r<e.length;r+=2){let a=Math.round((e[r]??0)/i)+8,c=Math.round((e[r+1]??0)/i)+8;n[r/2]=(a&15)<<4|c&15}return{data:n.buffer,layersQuantized:1,layersSkipped:0}}function Or(o){let t=new Float32Array(1),e=new Int32Array(t.buffer);t[0]=o;let n=e[0]??0,s=n>>16&32768,i=n>>12&2047,r=n>>23&255;return r<103?s:r>142?(s|=31744,s|=(r===255?0:1)&&n&8388607,s):r<113?(i|=2048,s|=(i>>114-r)+(i>>113-r&1),s):(s|=r-112<<10|i>>1,s+=i&1,s)}async function Rr(o,t){let e=o instanceof ArrayBuffer?o:await Jt(o),n=new Float32Array(e),s=n.length,r=[...n.map(Math.abs)].sort((d,u)=>d-u),a=Math.floor(t.sparsity*r.length),c=r[a]??0,l=0;for(let d=0;d<n.length;d++)Math.abs(n[d]??0)<c&&(n[d]=0,l++);return{modelData:n.buffer,actualSparsity:l/s,parametersPruned:l,totalParameters:s}}async function $r(o){let t=o instanceof ArrayBuffer?o.byteLength:o.metadata.sizeBytes,e=Math.floor(t/4);return{totalParameters:e,sizeBytes:t,layers:[],estimatedFlops:e*2,memoryRequirements:{weights:t,activations:t*.1,total:t*1.1}}}async function Ur(o,t={}){let{warmupRuns:e=3,runs:n=10}=t;for(let h=0;h<e;h++)await o();let s=[];for(let h=0;h<n;h++){let p=performance.now();await o(),s.push(performance.now()-p)}let r=s.reduce((h,p)=>h+p,0)/s.length,a=Math.min(...s),c=Math.max(...s),d=s.map(h=>Math.pow(h-r,2)).reduce((h,p)=>h+p,0)/s.length,u=Math.sqrt(d);return{avgTime:r,minTime:a,maxTime:c,stdDev:u,throughput:1e3/r,times:s}}async function Br(o,t){let e=o instanceof ArrayBuffer?o:await Jt(o);switch(t){case"json":let n=new Float32Array(e);return JSON.stringify(Array.from(n));case"binary":case"onnx":default:return e}}async function dc(){let o=await Ee();return Array.from(o.values()).some(t=>t)}async function uc(){let o=await Ee();return o.get("webgpu")?"webgpu":o.get("webnn")?"webnn":o.get("wasm")?"wasm":null}async function hc(o){let t=new ee;await Promise.all(o.map(async e=>{if(!await t.get(e)){let n=await fetch(e);n.ok&&await t.put(e,n)}}))}var qr="0.1.0";async function fc(){let o=await Ee();return{version:qr,runtimes:{webgpu:o.get("webgpu")??!1,webnn:o.get("webnn")??!1,wasm:o.get("wasm")??!1,auto:!0},features:["concurrent-execution","batch-processing","memory-management","model-caching","quantization"]}}export{fe as AudioPreprocessor,E as BasePipeline,Te as Cache,Nn as EMOTION_LABELS,Ve as EdgeFlowDebugger,x as EdgeFlowError,w as EdgeFlowTensor,M as ErrorCodes,ne as FeatureExtractionPipeline,ot as IMAGENET_LABELS,se as ImageClassificationPipeline,he as ImagePreprocessor,re as ImageSegmentationPipeline,st as InferenceCache,ke as InferenceScheduler,H as LoadedModelImpl,ce as MemoryManager,Ae as MemoryScope,ze as ModelCache,ee as ModelDownloadCache,Ce as ONNXRuntime,Bt as POPULAR_MODELS,We as PerformanceMonitor,V as RuntimeManager,rt as SENTIMENT_LABELS,te as SentimentAnalysisPipeline,Y as TextClassificationPipeline,pe as TextGenerationPipeline,P as Tokenizer,Oe as TransformersAdapterRuntime,qr as VERSION,Le as WASMRuntime,Pe as WebGPURuntime,_e as WebNNRuntime,dn as add,$r as analyzeModel,xs as analyzeModelDetailed,an as arange,gn as argmax,Ur as benchmark,hs as benchmarkMemory,ls as benchmarkSuite,vt as cancelPreload,At as clearModelCache,cs as compareBenchmarks,Tr as compose,yn as concat,En as configureOnnxAssets,$s as configureScheduler,Is as createAsciiHistogram,Vn as createAudioPreprocessor,Cn as createBasicTokenizer,Pn as createCache,jn as createFeatureExtractionPipeline,Wn as createImageClassificationPipeline,it as createImagePreprocessor,Zn as createImageSegmentationPipeline,tt as createONNXRuntime,Mr as createPipelines,Bn as createSentimentAnalysisPipeline,As as createTensorHeatmap,Un as createTextClassificationPipeline,Xn as createTextGenerationPipeline,Sn as createWASMRuntime,An as createWebGPURuntime,zn as createWebNNRuntime,It as deleteCachedModel,Vt as dequantizeFloat16,ut as dequantizeInt8,ys as dequantizeTensor,jt as dequantizeUint8,ks as disableDebugging,gt as div,ct as downloadConfig,lt as downloadModel,Ut as downloadTokenizer,bs as enableDebugging,Br as exportModel,Ms as exportModelAdvanced,ln as eye,Ht as float16ToFloat32,ds as formatBenchmarkResult,us as formatComparisonResult,Ts as formatTensorInspection,rs as fromHub,as as fromTask,sn as full,Vs as gc,vs as generateAsciiDashboard,Ss as generateDashboardHTML,Ee as getAvailableRuntimes,er as getBestRuntime,uc as getBestRuntimeType,kt as getCachedModel,Kt as getDebugger,qt as getDefaultModel,bn as getDeviceProfile,fc as getInfo,q as getMemoryManager,js as getMemoryStats,zt as getModelCacheStats,is as getModelInfo,Yt as getMonitor,Ln as getPipelineFactory,nr as getPluginMiddleware,Pt as getPluginPipeline,St as getPreloadStatus,Et as getPreloadedModel,Zs as getRuntimeManager,Ie as getScheduler,Fn as getTransformersAdapter,He as inspectTensor,ve as isModelCached,Nt as isOnnxAvailable,dc as isSupported,cn as linspace,sr as listPlugins,we as loadModel,v as loadModelData,F as loadModelFromBuffer,On as loadTokenizer,Rn as loadTokenizerFromHub,fn as matmul,mn as mean,os as modelExists,hn as mul,nn as ones,br as parallel,qe as pipeline,hc as preload,Tt as preloadModel,bt as preloadModels,Hn as preprocessText,Rr as prune,ws as pruneModel,Xt as pruneTensor,Pr as quantize,Gt as quantizeModel,gs as quantizeTensor,on as randn,rn as random,or as recommendModelVariant,kn as recommendQuantization,nt as registerAllBackends,N as registerPipeline,tr as registerPlugin,de as registerRuntime,Hs as release,Ke as relu,ir as resetDeviceProfile,Js as runBatchInference,je as runBenchmark,X as runInference,U as runInferenceNamed,vn as setOnnxModule,xn as setScheduler,Ye as sigmoid,O as softmax,Es as startMonitoring,Ds as stopMonitoring,un as sub,Xe as sum,pn as tanh,tn as tensor,rr as unregisterPlugin,Dn as useTransformersBackend,zs as visualizeModelArchitecture,Bs as withMemoryScope,qs as withMemoryScopeSync,mt as zeros};
